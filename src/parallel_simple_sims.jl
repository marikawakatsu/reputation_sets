...
function main(args)
    ...
    @everywhere function run_worker(inputs, results)
        ...
        while true
			...
			if pard["permitted_strategies"] == "all"
				permitted_strategies = [1,2,3,4,5]
			else
			...
			pop = Population(sets, game, permitted_strategies .- 1, false)
			...
			strategy_freqs_array = zeros(Float64, num_samples, 5)
			fitness_means_array = zeros(Float64, num_samples, 5)
			for g in 1:num_samples
    ...
    cols = push!(sort(collect(keys(pars))),
                 ["rep", "strat_fitness_means", "fitness_means", "strategy_freqs", "total_cooperation", "set_reputation_means", "reputation_means", "reputation_stds", "seed1", "seed2", "seed3", "seed4"]...)
...
main(["--input", "submit/simplified_M_K_redo_bugfix_long.json"])
